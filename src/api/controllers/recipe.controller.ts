...
const FilterValidation: ValidationChain[] = [
    ...
    query('difficulty').if(query('difficulty').exists()).isIn(Object.keys(difficulty)),
    query('media').if(query('media').exists()).isIn(Object.keys(media)),
    query('taste').if(query('taste').exists()).isIn(Object.keys(taste)),
...
const RecipeValidation: ValidationChain[] = [
    ...
    body('difficulty').isIn(Object.keys(difficulty)),
    body('media').isIn(Object.keys(media)),
    body('taste').isIn(Object.keys(taste)),
    body('vegetarian').isBoolean(),

    check('ingredients.*.name').isString(),
    check('ingredients.*.amount').isNumeric(),
    check('ingredients.*.unit').isIn(Object.keys(unit)),
];
